  ...
  let rec loop = function
    | Sess.Out (role, conts) ->
      let flds =
        List.map
          (fun (lab, (_typ, cont)) ->
            Ast_helper.Of.tag ~loc { txt = lab; loc } (out (loop cont)))
          conts
      in
      Ast_helper.Typ.object_ ~loc
        [ Ast_helper.Of.tag ~loc { txt = role; loc }
    ...
    | Inp (role, conts) ->
      let inp conts =
        let flds =
          List.map
            (fun (lab, (_typ, cont)) ->
              Ast_helper.Rf.tag ~loc { txt = lab; loc } false
                [ Ast_helper.Typ.tuple ~loc
                    [ Ast_helper.Typ.any ~loc (); loop cont ]
                ])
            conts
        in
        Ast_helper.Typ.constr ~loc
          { txt = Longident.Lident "inp"; loc }
          [ Ast_helper.Typ.variant ~loc flds Asttypes.Open None ]
      in
      Ast_helper.Typ.object_ ~loc
        [ Ast_helper.Of.tag ~loc { txt = role; loc } (inp conts) ]
        Asttypes.Open
    | End ->
      Ast_helper.Typ.constr ~loc { txt = Longident.Lident "unit"; loc } []
    | Rec (var, st) -> Ast_helper.Typ.alias ~loc (loop st) var
    | Var var -> Ast_helper.Typ.var ~loc var
    | Err msg -> chvec_errtyp msg
  in
  loop
...
let rec to_session_type (vars : string list) :
  ...
  let input_pair exit ty =
    ...
    | Ptyp_tuple [ pld; cont ] ->
      let exit =
        { f =
            (fun cont ->
              exit.f { ty with ptyp_desc = Ptyp_tuple [ pld; cont ] })
        }
      in
  ...
  let rtag exit r =
    ...
    | Rtag (lab, opt, typs) ->
      if List.length typs = 1 then
        let exit =
          { f =
              (fun typ -> exit.f { r with prf_desc = Rtag (lab, opt, [ typ ]) })
          }
        in
        ...
        exit.f
          { r with
            prf_desc =
              Rtag (lab, opt, [ chvec_errtyp "should_not_be_a_conjunction" ])
          }
    | _ ->
      (* ignore *)
      None
  ...
  let input_variant exit typ =
    ...
    | Ptyp_variant (flds, x, y) ->
      let exit =
        { f =
            (fun flds ->
              exit.f { typ with ptyp_desc = Ptyp_variant (flds, x, y) })
        }
      in
      map_all exit rtag flds
    | _ -> exit.f (chvec_errtyp "should_be_a_polymorphic_variant_type")
  in
  (* ('pld * 'cont) out *)
  let output exit typ =
    match typ.ptyp_desc with
    | Ptyp_constr (name, [ pld; cont ])
      when Util.string_of_longident name.txt = "out" ->
      let exit =
        { f =
            (fun cont ->
              exit.f { typ with ptyp_desc = Ptyp_constr (name, [ pld; cont ]) })
        }
      in
      (string_of_ptyp pld, to_session_type vars exit cont)
    | _ -> exit.f (chvec_errtyp "should_be_an_output_type")
  in
  (* <lab: typ> *)
  let otag exit o =
  ...
  let method_or_inp exit role typ =
    ...
    | Ptyp_object (flds, x) ->
      let exit =
        { f =
            (fun flds -> exit.f { typ with ptyp_desc = Ptyp_object (flds, x) })
        }
      in
      Out (role.Location.txt, map_all exit otag flds)
    | Ptyp_constr (name, [ variant ])
      when Util.string_of_longident name.txt = "inp" ->
      let exit =
        { f =
            (fun variant ->
              exit.f { typ with ptyp_desc = Ptyp_constr (name, [ variant ]) })
        }
      in
      Inp (role.txt, input_variant exit variant)
    | _ -> exit.f (chvec_errtyp "should_be_inp_or_output_object")
  in
  fun exit (ty : core_type) ->
            ...
            (fun typ ->
              exit.f
                { ty with
                  ptyp_desc =
                    Ptyp_object
                      ([ { pof with pof_desc = Otag (role, typ) } ], flag)
                })
        }
      in
    ...
    (* error *)
    | _ -> exit.f (chvec_errtyp "should_be_a_role_object_or_a_unit_type")
...
let to_session_type ty =
  ...
  | FormatError ty -> Either.Right ty
